...
class AdminPaymentMethodController{
	...
	public function display(){
		...
		$zController->getView('/backend/payment-method/display.php');
	}
	...
	public function add(){		
		...
		if(strcmp($_SERVER['REQUEST_METHOD'], 'POST')==0){
			$model = $zController->getModel('/backend','AdminPaymentMethodModel');
			$model->save_item();
			$url = 'admin.php?page=' . $_REQUEST['page'] . '&msg=1';
			wp_redirect($url); 
		}
		$zController->getView('/backend/payment-method/form.php');
	}
